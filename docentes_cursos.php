...
<?php
    function initial(){
        $title="Asignacion de docentes al curso";
        $_PAGE = array();
        $_PAGE ['title'] = $title;
        $_PAGE ['links'] = null;
        ...
        $sql_curso = "SELECT tblcurso.nombre as 'nombre_curso', tblmateria.nombre as 'nombre_materia', tbldocente.nombres, tbldocente.apellidos FROM tblcurso INNER JOIN tbldocente on tbldocente.id_docente = tblcurso.id_docente INNER JOIN tblmateria on tblmateria.id_materia = tblcurso.id_materia";
    ...
    <div class="wrapper wrapper-content  animated fadeInRight">
        <div class="row">
                ...
                <div class="ibox ">
                    ...
                    <div class="ibox-content">
                        <div class="row focuss"><br>
                            ...
                            <div class="form-group col-md-12">
                                <label id='buscar_habilitado'>Buscar Docente</label>
                                <input type="text" id="docente_buscar" name="docente_buscar"
                                    class="form-control usage" placeholder="Ingrese Descripcion del Docente"
                                                            ...
                                                            <table id="inventable1">
                                                                <thead>
                                                                    <tr class="row100 head">
                                                                        ...
                                                                        <th class='success  cell100 column35'>DOCENTE</th>
                                                                        <th class='success  cell100 column35'>USUARIO</th>
            ...
            <?php
                  ...
                  echo "<script src='js/funciones/funciones_docentes_cursos.js'></script>";
...
function consultar_existencia(){
    ...
    $sql_existencia = "SELECT * FROM tbldocente_curso WHERE id_docente = '$id_docente' AND id_curso = '$id_curso' AND deleted is NULL";
    $query_existencia = _query($sql_existencia);
    if(_num_rows($query_existencia) > 0){
        $xdatos['typeinfo'] = "Error";
        $xdatos['msg'] = "El docente ya esta agregado al curso!";
    }
    else{
...
<?php
    function traer_docentes(){
        $id_curso = $_POST['id_curso'];
        $sql_estudiante_curso = "SELECT * FROM tbldocente INNER JOIN tbldocente_curso ON tbldocente.id_docente = tbldocente_curso.id_docente WHERE tbldocente_curso.deleted is NULL AND tbldocente.deleted is NULL AND tbldocente_curso.id_curso = '$id_curso'";
        ...
        if(_num_rows($query_estudiante_curso) > 0){
            while($row = _fetch_array($query_estudiante_curso)){
                $nombre_docente = $row['nombres']." ".$row['apellidos'];
                $codigo = $row['codigo'];
...
<?php 
    function insertar(){
        ...
        if($delete_total && $error == 0){
            $xdatos['typeinfo'] ="Success";
            $xdatos['msg'] ="Docentes del curso actualizados correctamente!";
            _commit();
        }
        else{
            $xdatos['typeinfo'] ="Error";
            $xdatos['msg'] ="No se pudieron actualizar los docentes del curso!";
            _rollback();
        }
        echo json_encode($xdatos);
    }
